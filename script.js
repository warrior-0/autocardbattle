    ...
    try {
        if (isSignupMode) {
            ...
            if (!response.ok) {
                ...
                if (errorMsg.includes("중복되는 닉네임")) {
                    // 중복 시 Firebase 회원 탈퇴 처리
                    await result.user.delete();
                    return alert("중복되는 닉네임이 있습니다.");
                }
...
function normalizeBattleHand(handFromServer, targetSize = 2) {
    const source = Array.isArray(handFromServer) ? handFromServer.filter(Boolean) : [];
    const uniqueHand = [];

    source.forEach(type => {
        if (!uniqueHand.includes(type)) uniqueHand.push(type);
    });

    const deckPool = (currentUser && currentUser.selectedDeck)
        ? currentUser.selectedDeck.split(',').map(d => d.trim()).filter(Boolean)
        : [];

    const fallbackPool = allDice.map(d => d.diceType);
    const candidatePool = [...new Set([...deckPool, ...fallbackPool])];

    for (const type of candidatePool) {
        if (uniqueHand.length >= targetSize) break;
        if (!uniqueHand.includes(type)) uniqueHand.push(type);
    }

    return uniqueHand.slice(0, targetSize);
}
...
function handleBattleMessage(data) {
    switch(data.type) {
        case "GAME_START":
            ...
            myHand = normalizeBattleHand(data.nextHand, 2);
            placementCount = 0;
            renderHand();
            startBattleTimer();
            break;
        ...
        case "REVEAL":
            ...
            // 1. 맵 전체 공개 (이전 라운드 배치를 유지하면서 이번 배치 반영)
            mergePlacements(data.allPlacements);
            renderFullMap(persistedPlacements, true);
...
function animateProjectile(sx, sy, tx, ty, type, onHit) {
    // 시작 타일과 목표 타일이 같으면 투사체 생략
    if (sx === tx && sy === ty) {
        if (onHit) onHit();
        return; // 더 이상 투사체 생성/애니메이션 하지 않음
    }
...
function renderFullMap(placements, isBattleMode) {
    ...
    placements.forEach(p => {
        ...
        if (tile) {
            ...
            const unitDiv = document.createElement('div');
            unitDiv.className = `dice-unit ${p.diceType} ${ownershipClass}`;
    ...
    syncMapDataWithPlacements(placements);
}

function mergePlacements(newPlacements) {
    if (!Array.isArray(newPlacements)) return;

    newPlacements.forEach(p => {
        const idx = persistedPlacements.findIndex(prev => prev.x === p.x && prev.y === p.y);
        if (idx >= 0) {
            persistedPlacements[idx] = p;
        } else {
            persistedPlacements.push(p);
        }
    });
}

function syncMapDataWithPlacements(placements) {
    ...
    placements.forEach(p => {
        const tile = mapData.find(t => t.x === p.x && t.y === p.y);
        if (!tile) return;
        tile.hasDice = true;
        tile.diceType = p.diceType;
        tile.level = p.level || 1;
    });
}
...
function startNextRound(nextHand, nextTurnVal) {
    ...
    placementCount = 0;
    selectedDiceFromHand = null;
    myHand = normalizeBattleHand(nextHand, 2);
