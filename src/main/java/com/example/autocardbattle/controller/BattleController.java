...
public class BattleController {
    ...
    // AI전 메타 정보
    private static final Map<String, AiMatchContext> aiMatches = new ConcurrentHashMap<>();

    public record AiMatchContext(String humanUid, String aiUid, String aiType) {}
    ...
    public ResponseEntity<?> cancelMatch(@RequestParam String userUid) {
        synchronized (matchingQueue) {
            ...
            if (roomId != null && isAiRoom(roomId)) {
    ...
    public static void removeRoomData(String roomId) {
        if (roomId != null) {
            ...
            roomReadyStatus.remove(roomId);
            aiMatches.remove(roomId);
    ...
    public ResponseEntity<?> requestMatch(
        ...
        synchronized (matchingQueue) {
            ...
            if ("AI".equalsIgnoreCase(mode)) {
                ...
                String resolvedAiType = (aiType == null || aiType.isBlank()) ? "ALPHA" : aiType.toUpperCase(Locale.ROOT);
                String aiUid = "AI_" + resolvedAiType + "_" + roomId;

                userRooms.put(userUid, roomId);
                userRooms.put(aiUid, roomId);
                roomMaps.put(roomId, mapRepository.findRandomMap());
                roomReadyStatus.put(roomId, ConcurrentHashMap.newKeySet());
                aiMatches.put(roomId, new AiMatchContext(userUid, aiUid, resolvedAiType));

                return ResponseEntity.ok(Map.of("roomId", roomId, "mode", "AI", "aiType", resolvedAiType));
            }
    ...
    public static boolean isAiRoom(String roomId) {
        return aiMatches.containsKey(roomId);
    }

    public static AiMatchContext getAiMatchContext(String roomId) {
        return aiMatches.get(roomId);
    }
}
