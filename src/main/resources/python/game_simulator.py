...
class GameSimulator:
    ...
    def _resolve_round(self) -> Tuple[int, Dict[str, float]]:
        ...
        survivor_diff = (survivors["player"] - survivors["enemy"]) * 0.02 # 스케일 조정
        
        # 기본 승패 보상
        win_reward = 3.0 if winner == 1 else (-3.0 if winner == -1 else -0.1)
        
        # 최종 보상 합산
        player_reward = win_reward + dmg_diff + hp_diff + survivor_diff
        player_reward = np.clip(player_reward, -5, 5)
