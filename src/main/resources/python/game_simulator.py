...
class DiceCatalogLoader:
    @staticmethod
    def load(seed_file: str | Path | None = None) -> Tuple[List[DiceStat], str]:
        database_rows = DiceCatalogLoader._load_from_db()
        if database_rows:
            return database_rows, "db"
        return DiceCatalogLoader._load_from_seed_file(seed_file), "seed_fallback"

    @staticmethod
    def _load_from_db() -> List[DiceStat]:
        if not all(os.getenv(key) for key in DB_ENV_KEYS):
            return []
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT dice_type, name, hp, damage, dice_range, aps, description, color
                    FROM dice_master
                    ORDER BY id ASC
                    """
                )
        ...
        return [
            DiceStat(
                ...
                name=row.get("name") or row["dice_type"],
                hp=int(row["hp"]),
                damage=int(row["damage"]),
                attack_range=int(row["dice_range"]),
                aps=float(row["aps"]),
...
class GameSimulator:
    ...
    The simulator reads dice definitions from the application's data source.
    When live DB access is unavailable in the training environment, it falls
    back to the Spring boot seed definitions so the same dice schema is still
    used.
    """

    def __init__(self, seed: int | None = None, dice_catalog: Sequence[DiceStat] | None = None):
        self.random = random.Random(seed)
        loaded_catalog, catalog_source = DiceCatalogLoader.load() if dice_catalog is None else (list(dice_catalog), "provided")
